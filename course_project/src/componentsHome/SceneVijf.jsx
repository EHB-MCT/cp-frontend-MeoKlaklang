...
export default function SceneVijf() {
	...
	const sterren = [stip, stip, bigStar, star2, star3, stip, stip, bigStar, star2, star3, stip, stip, bigStar, star2, star3, stip, stip, bigStar, star2, star3];
		...
		<div className="parallax-wrapper" onMouseEnter={handleHover}>
			...
			<Parallax pages={2}>
				...
				<ParallaxLayer offset={0.5} speed={1}>
					<>
						{[1, 2, 3, 4, 5, 6].map((i) => (
							<motion.img key={i} src={vlinder2} alt={`vlinder ${i}`} className={`vlinder-vijf vlinder-${i}`} {...fladderAnimatie} />
						))}
					</>
				</ParallaxLayer>
				...
				<ParallaxLayer offset={1} speed={1.5}>
					<>
						{sterren.map((src, index) => (
							<motion.img key={index} src={src} alt={`ster ${index + 1}`} className={`ster ster-${index + 1}`} {...genPulse()} />
						))}
					</>
				</ParallaxLayer>
			</Parallax>
